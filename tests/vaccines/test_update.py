...
@pytest.mark.asyncio
async def test_patch_rejects_expiration_date_before_existing_vaccination_date(
        authenticated_client,
        vaccine_in_db,
):
    """
    ...
    # Ожидаемое поведение API - клиентская ошибка, а не падение сервера
    assert response.status_code == 422
    assert "expiration_date должна быть позже, чем vaccination_date" in response.text
